...
/**********
* a helper function to calculate the size of the map in a waveform header
* @return 
...
//typedefs
	...
	uint32_t map_size(std::map<uint32_t, std::vector<T> > mymap) {
		...
		for (std::map<uint32_t, std::vector<T> >::const_iterator it = mymap.begin(); it != mymap.end(); it++) {
	...
	uint32_t map_size(std::map<uint32_t, T > mymap) {
		...
		for (std::map<uint32_t, T >::const_iterator it = mymap.begin(); it != mymap.end(); it++) {
			size += sizeof(it->first);
			size += sizeof(T);
		}
		return  size;
